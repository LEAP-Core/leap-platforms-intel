...
// depends on RAM_STYLE and mode. Mode 0 can only be mapped to
// distributed RAM. Mode 1/2/3 can be mapped to either distributed or block
// RAM. There are three supported values for RAM_STYLE.
...
parameter RAM_MODE      =2'd3;       // GRAM read mode
...
(* `GRAM_STYLE = RAM_STYLE *)
reg [BUS_SIZE_DATA-1:0] ram [(2**BUS_SIZE_ADDR)-1:0];

reg [BUS_SIZE_ADDR-1:0] raddr_q;        // synthesis `NO_RETIMING
reg [BUS_SIZE_DATA-1:0] dout;           // synthesis `NO_RETIMING
        ...
        `else   // VENDOR_ALTERA
         ...
         begin
                dout = ram[raddr_q];
                                                //synthesis translate_off
                if(driveX)
                dout = 'hx;                     //synthesis translate_on
         end         
        `endif
