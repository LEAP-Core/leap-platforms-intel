...
class Verilog():
  ...
  def __init__(self, moduleList, isPrimaryBuildTarget):
    ...
    vexe_gen_command = \
        BSC + ' ' + BSC_FLAGS_VERILOG + ' -vdir ' + vexe_vdir + ' -simdir ' + vexe_vdir + ' -bdir ' + vexe_vdir +' -p +:' +  ALL_LIB_DIRS_FROM_ROOT + ' -vsearch +:' + vsearch_path + ' ' + \
        ' -o $TARGET' 
    ...
    # ASE simulation side needs SIM_SIDE defined
    vexe_gen_command += ' -Xc -DSIM_SIDE -Xv -CFLAGS -Xv -DSIM_SIDE'
    # ASE needs to include .h files for DPI-C sources
    vexe_gen_command += ' -Xv -CFLAGS -Xv -I' + ase_sw_dir
